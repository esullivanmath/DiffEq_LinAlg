\chapter{Qualitative and Numerical Methods}
...
\subsection{Phase Line and Slope Field Analysis}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item[(b)] Based on the slope field in Figure \ref{fig:slope_field_1}, your solution plots
            from part (a), and the phase plot in Figure \ref{fig:7.3.phase}, why are we
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[axis lines=center, xmin=-0.6, xmax=6, ymin=-2.5, ymax=2.5, xlabel={$y$},
                ...
                \draw (axis cs:1,0.5) node{$\frac{dy}{dt}>0$};
                \draw (axis cs:5,-1.5) node{$\frac{dy}{dt}<0$};
                \draw[thick, red, ->] (axis cs:4.5,1)
...
\begin{problem}
    ...
    inspiration.
    \begin{enumerate}
        \item[(a)] To find the equilibrium for a differential equation,
...
\begin{problem}[Crowd Sourced Qualitative Analysis]
    Supplies Needed:
    \begin{itemize}
        \item toothpicks
        \item yarn
        \item a large table (or the floor)
        \item tape
        \item scissors
        \item sticky notes
    \end{itemize}

    Making slope fields by hand is rather cumbersome, but only ever relying on technology can
    detract from what you are actually doing in a slope field.  In this problem you will
    work with a team of four to build slope fields, phase plots, and solution plots using
    toothpicks and yarn.  
    \begin{enumerate}
        \item[(a)] Start by setting up two coordinate planes.  The first one will be $y$
            vs $t$.  The second one will be $dy/dt$ vs $y$.  In each case we're only
            interested in the first quadrant.  Use your yarn to mark the axes and then
            tape them in place. (Make these coordinate planes a few feet by a few feet)
        \item[(b)] We'll start with the differential equation 
            \[ \frac{dy}{dt} = \frac{1}{2} y \left( 3-y \right). \]
            With your team do the following tasks on this DE.  Divide the tasks among your
            group members.
            \begin{enumerate}
                \item[(i)] Use the toothpicks to mark the slopes in the slope field (on
                    the $y$ vs $t$ plot) for every integer unit of $t$ (from $0$ to $4$)
                    and every integer value of $y$ (from $0$ to $5$). (that should be 30
                    toothpicks!)
                \item[(ii)] Use the yarn to show solution curves with initial conditions
                    $y(0) = 0$, $y(0) = 0.1$, $y(0) = 1$, $y(0) = 2$, $y(0)=3$, $y(0) =
                    4$, and $y(0) = 5$.  Feel free to cut the yarn.
                \item[(iii)] In the $dy/dt$ vs $y$ plot use the yarn to create the phase
                    plot.  
                \item[(iv)] Indicate the location and stability of the equilibrium points
                    with the sticky notes on both plots.
                \item[(v)] Take a picture of both of your plots when they're complete.
                    Your instructor will tell you where to upload the photos.
            \end{enumerate}
        \item[(c)] Create a differential equation that has 1 stable equilibrium, 1
            unstable equilibrium, and 1 semi-stable equilibrium.  Then work with your
            group to repeat part (b) of this problem with your new differential equation.
    \end{enumerate}

    Keep in mind that a positive slope points up and to the right, while a negative slope
    points down and to the right.  Also, a slope with an absolute value of 1 has an angle
    of $45^\circ$, a slope with an absolute value less than 1 is more shallow, and a slope
    with an absolute value greater than 1 is more steep.
...
\begin{problem}[The Spherical Flame]
    ...
    \begin{enumerate}
        ...
        \item[(b)] Having two parameters in the model is a bit annoying so the following
            ...
            parameter.  In this case, let $R$ be a new variable defined as the radius of
        ...
        \item[(d)] Clearly explain how what you're seeing in the plots relates to what is
